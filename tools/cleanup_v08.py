...
old_core = Path("R/CATER_MR_core_v05.R")
new_core = Path("R/CATER_MR_core.R")
s = old_core.read_text()

s = s.replace('.CATER_VERSION <- "0.8.0"', '.CATER_VERSION <- "0.8.1"', 1)
s = s.replace('# Significant-IV + GRN-gating + LD-aware MR. COJO is retained only as a legacy helper.\n', '# Significant-IV + GRN-gating + LD-aware MR.\n', 1)

start = s.index('.cater_remove_prefix_outputs <- function')
end = s.index('.cater_reorient_ld <- function', start)
s = s[:start] + s[end:]
...
new_sig = '''cater_mr <- function(grn,eqtl_dir,outcome,gene_annotation=NULL,ld_bfile,
                     ...
                     input_manifest=NULL,strict_input_contract=FALSE) {'''
if old_sig not in s:
    raise SystemExit("cater_mr signature not found")
s = s.replace(old_sig, new_sig, 1)

old_validation = '  if(!is.logical(enable_ld_diagnosis)||length(enable_ld_diagnosis)!=1L||is.na(enable_ld_diagnosis)).cater_stop("enable_ld_diagnosis must be TRUE or FALSE")\n'
if old_validation not in s:
    raise SystemExit("obsolete LD-diagnosis validation not found")
s = s.replace(old_validation, '', 1)
s = s.replace('cojo_threads', 'ld_threads')
s = s.replace('  trans_data_mode<-.cater_resolve_trans_contract(manifest,trans_eqtl,instrument_p,trans_reporting_p)\n',
              '  .cater_resolve_trans_contract(manifest,trans_eqtl,instrument_p,trans_reporting_p)\n', 1)

new_core.write_text(s)
old_core.unlink()

Path("CATER_MR.R").write_text('''# CATER-MR entrypoint\nsource(file.path("R", "CATER_MR_core.R"), local = FALSE)\n''')

eq = Path("CATER_EQTL_INPUT.R")
eqs = eq.read_text()
head_end = eqs.index('.cater_eqtl_input_stop <- function')
eqs = '''# CATER-MR full-summary eQTL table adapter\n# Materializes merged full-summary gene associations into <GENE>.txt.gz caches.\n\n''' + eqs[head_end:]
eq.write_text(eqs)

smoke = Path("tests/smoke.R")
ts = smoke.read_text()
a = ts.index('# Manc-COJO chromosome-block LD parser.')
b = ts.index('# Allele-to-LD orientation flips beta and EAF when required.', a)
ts = ts[:a] + ts[b:]
ts = ts.replace('CATER-MR v0.5 core-estimator + scMORE adapter regression tests passed',
                'CATER-MR core-estimator + scMORE adapter regression tests passed')
smoke.write_text(ts)

hard = r'''source("CATER_MR.R")
...
'''
Path("tests/core_hardening_smoke.R").write_text(hard)
Path("tests/v06_hardening_smoke.R").unlink()
cojo_test = Path("tests/cojo_allele_read_regression.R")
if cojo_test.exists(): cojo_test.unlink()

v08 = Path("tests/v08_significant_trans_smoke.R")
v = v08.read_text()
v = v.replace('legacy_tail <- c("outdir","drop_palindromic","verbose","primary_policy")\ni <- match("outdir",fml); stopifnot(identical(fml[i:(i+3L)],legacy_tail))',
'''legacy_tail <- c("outdir","drop_palindromic","verbose","primary_policy")
i <- match("outdir",fml); stopifnot(identical(fml[i:(i+3L)],legacy_tail))
stopifnot("ld_threads" %in% fml)
stopifnot(!any(c("manc_cojo_bin","cojo_p","cojo_wind_kb","cojo_collinear","cojo_threads",
                 "enable_ld_diagnosis","ld_diag_loglr","ld_diag_abs_z") %in% fml))''', 1)
v08.write_text(v)

wf = Path(".github/workflows/r-smoke.yml")
w = wf.read_text()
w = w.replace('          Rscript -e "parse(file=\'CATER_MR.R\')"\n',
              '          Rscript -e "parse(file=\'CATER_MR.R\')"\n          Rscript -e "parse(file=\'R/CATER_MR_core.R\')"\n', 1)
w = w.replace('      - name: Run v0.6 hardening tests\n        run: Rscript tests/v06_hardening_smoke.R\n',
